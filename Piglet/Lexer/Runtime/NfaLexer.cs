...
{
    internal class NfaLexer<TContext, T> : LexerBase<TContext, T, HashSet<NFA.State>>
    {
        private readonly NFA nfa;
        private readonly Tuple<NFA.State, Tuple<int, Func<TContext, string, T>>>[] actions;

        public NfaLexer(NFA nfa, IEnumerable<NFA> nfas, List<Tuple<string, Func<TContext, string, T>>> tokens, int endOfInputTokenNumber)
            : base(endOfInputTokenNumber)
        {
            this.nfa = nfa;
            actions = nfas.Select((n, i) => new Tuple<NFA.State, Tuple<int, Func<TContext, string, T>>>(n.States.Single(f => f.AcceptState), new Tuple<int, Func<TContext, string, T>>( i,
                i < tokens.Count ? tokens[i].Item2 : null))).ToArray();
        }

        protected override Tuple<int, Func<TContext, string, T>> GetAction(HashSet<NFA.State> state)
        {
            ...
            return action != null && action.Item2.Item2 != null ? action.Item2 : new Tuple<int, Func<TContext, string, T>>(int.MinValue, null);
        }
