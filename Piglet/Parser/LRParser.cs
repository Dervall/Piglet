...
namespace Piglet.Parser
{
    ...
    {
        private readonly IParseTable<T> _parseTable;
        ...
        public IParseTable<T> ParseTable => _parseTable;
        ...
        private T Parse(ILexerInstance<T> lexerInstance)
        {
            ...
            (int index, T value) input = lexerInstance.Next();
            ...
            while (true)
            {
                ...
                int action = _parseTable.Action[state, input.index];
                if (action >= 0)
                {
                    ...
                    // Shift
                    parseStack.Push(input.index);   // Push token unto stack
        ...
        private IEnumerable<string> GetExpectedTokenNames(int state) => _terminalDebugNames.Where((t, i) => _parseTable.Action[state, i] != short.MinValue);
